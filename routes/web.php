...
function getFirestore() {
    // 1. Production Build (Render Dashboard Environment Variable)
    $firebaseJson = env('FIREBASE_JSON');

    if (!empty($firebaseJson)) {
        // Clean up escaped newlines, hidden carriage returns, or spaces added by the dashboard copy-paste
    ...
    // 2. Local Fallback (For working on your local machine)
    ...
    // 3. Fail-safe backup block to prevent 500 boot errors
    return new FirestoreClient([
        'projectId' => 'furecipe',
        'transport' => 'rest'
    ]);
